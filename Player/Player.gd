...
onready var remoteTranform2D:= $RemoteTransform2D
...
func connect_camera(camera):
	...
	remoteTranform2D.remote_path = camera_path
...
func fast_fall():
	if velocity.y > 0 :
		velocity.y += moveData.ADDITION_FALL_GRAVITY
...
func input_jump():
	if Input.is_action_just_pressed("ui_up") or buffer_jump:
		SoundPlayer.play_sound(SoundPlayer.JUMP)
		velocity.y = moveData.JUMP_FORCE
		buffer_jump = false
